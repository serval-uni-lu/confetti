...
if __name__ == "__main__":
    ...
    for dataset, params in checkpoint.items():
        ...
        model_path = str(cfg.TRAINED_MODELS_DIR / dataset / f"{dataset}_fcn.keras")
        ...
        for alpha in tqdm(alphas.copy(),
            ...
            # save CFs + timing
            ces_naive.to_csv(ce_dir / f"confetti_naive_{dataset}_fcn_alpha_{alpha}.csv", index=False)
            ces_optimized.to_csv(ce_dir / f"confetti_optimized_{dataset}_fcn_alpha_{alpha}.csv", index=False)
            # save execution time
            pd.DataFrame([{
                'Dataset': dataset,
                'Alpha': alpha,
        ...
        for theta in tqdm(thetas.copy(),
            ...
            # save CFs + timing
            ces_naive.to_csv(ce_dir / f"confetti_naive_{dataset}_fcn_theta_{theta}.csv", index=False)
            ces_optimized.to_csv(ce_dir / f"confetti_optimized_{dataset}_fcn_theta_{theta}.csv", index=False)
            pd.DataFrame([{
                'Dataset': dataset,
                'Alpha': cfg.FIXED_ALPHA,
                'Theta': theta,
