...
def run_proximity_metric_experiment(model_name: str = "fcn"):
    ...
    for dataset in tqdm(cfg.DATASETS, desc="Datasets", unit="dataset"):
        ...
        for metric in tqdm(["softdtw", "ctw", "dtw", "euclidean"], desc="Proximity Metrics", unit="metric"):
            ...
            ces_optimized.to_csv(results_dir / f"{dataset}_{metric}_optimized.csv", index=False)

            if metric == "dtw":
                small_window = int(X_train.shape[1] * 0.20)
                medium_window = int(X_train.shape[1] * 0.50)

                for dtw_window in tqdm([small_window, medium_window], desc="DTW Windows", unit="window"):
                    start_time = time.time()
                    ...
                    ces_naive["Solution"] = ces_naive["Solution"].apply(array_to_string)
                    ces_naive.to_csv(results_dir / f"{dataset}_{metric}_{dtw_window}_naive.csv", index=False)

                    ces_optimized["Solution"] = ces_optimized["Solution"].apply(array_to_string)
                    ces_optimized.to_csv(results_dir / f"{dataset}_{metric}_{dtw_window}_optimized.csv",index=False)
