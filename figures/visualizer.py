...
def boxplot_single_tradeoff(
    data: pd.DataFrame,
    metric1: Optional[str] = "sparsity",
    metric2: Optional[str] = "confidence",
    parameter: Optional[str] = "alpha",
) -> None:
    """
    ...
    if metric1 not in valid_metrics1 or metric2 not in valid_metrics2:
        raise ValueError(
            "Invalid metrics. metric1 must be in ['sparsity', 'confidence'], "
...
def boxplot_all_tradeoffs(
    results_alpha: pd.DataFrame, results_theta: pd.DataFrame
) -> None:
    """
...
# Figure Trade-offs
def boxplot_all_tradeoffs_by_model(
    results_alpha: pd.DataFrame, results_theta: pd.DataFrame
) -> None:
    tradeoffs = [
        (
...
def plot_counterfactual_with_cam(
    sample: np.ndarray,
...
def plot_counterfactual_with_cam_as_line(
    sample: np.ndarray,
    counterfactual: np.ndarray,
    weights: np.ndarray = None,
    dimension: int = 0,
    ...
    if weights is not None:
        ax.plot(
            weights, label="CAM Weights", color=cam_color, linewidth=2, linestyle=":"
        )
...
def plot_method_comparison_with_cam(
    sample, counterfactuals_dict, cam, dimension_idx, title=""
):
    """
    ...
    for (method, cf_array), linestyle, color in zip(
        counterfactuals_dict.items(), linestyles, colors
    ):
        cf_reshaped = cf_array.T
        diffs = np.where(sample_reshaped[dimension_idx] != cf_reshaped[dimension_idx])[
            0
        ]
        if diffs.size > 0:
            start_diff, end_diff = diffs[0], diffs[-1]
            ax1.plot(
                range(start_diff, end_diff + 1),
                cf_reshaped[dimension_idx, start_diff : end_diff + 1],
                label=method,
                linestyle=linestyle,
                color=color,
                linewidth=2,
            ...
            if start_diff > 0:
                ax1.plot(
                    [start_diff - 1, start_diff],
                    [
                        sample_reshaped[dimension_idx, start_diff - 1],
                        cf_reshaped[dimension_idx, start_diff],
                    ],
                    color=color,
                    linewidth=2,
                )
            if end_diff < sample_reshaped[dimension_idx].size - 1:
                ax1.plot(
                    [end_diff, end_diff + 1],
                    [
                        cf_reshaped[dimension_idx, end_diff],
                        sample_reshaped[dimension_idx, end_diff + 1],
                    ],
                    color=color,
                    linewidth=2,
                )
...
def plot_method_comparison_separated_with_cam(
...
def visualize_counterfactuals(
    sample: np.ndarray, counterfactual: np.ndarray, dimension: int = 0
) -> None:
    """
    ...
    (original_line,) = ax.plot(
        sample_reshaped[dimension], label="Original", color="#64cdc0", linewidth=2
    )
        ...
        if diffs.size > 0:
            ...
            if start_diff > 0:
                ax.plot(
                    ...
                    [
                        sample_reshaped[dimension, start_diff - 1],
                        cf_reshaped[dimension, start_diff],
                    ],
                    color="salmon",
                    linewidth=2,
                )
            ...
            if end_diff < sample_reshaped[dimension].size - 1:
                ax.plot(
                    [end_diff, end_diff + 1],
                    [
                        cf_reshaped[dimension, end_diff],
                        sample_reshaped[dimension, end_diff + 1],
                    ],
                    color="salmon",
                    linewidth=2,
