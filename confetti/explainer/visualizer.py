...
def plot_counterfactual_comparison(
    naive_csv, optimized_csv, figsize=(20, 20), constrained_layout=True
):
    """
    ...
    fig, axs = plt.subplots(
        8, 5, figsize=figsize, constrained_layout=constrained_layout
    )
    axs = axs.ravel()
    ...
    for i, test_instance in enumerate(grouped_original.groups.keys()):
        ...
        ax.scatter(
            group_original["Sparsity"],
            group_original["Precision"],
            s=60,
            color=optimized_color,
            label="Optimized",
        )
        ax.scatter(
            group_naive["Sparsity"],
            group_naive["Precision"],
            s=60,
            marker="s",
            color=naive_color,
            label="Naive",
...
def plot_mean_sparsity_confidence(
    results_df, figsize=(14, 6), naive_width=0.55, optimized_width=0.33
):
    """
    ...
    axes[1].bar(
        results_df["Dataset"],
        results_df["Confidence Optimized"],
        width=optimized_width,
        label="Optimized",
        align="edge",
        color=optimized_color,
...
def visualize_time_series(
    series, num_dimensions_to_show=3, line_color="#FF595A", line_width=2
):
    """
    ...
    fig, axes = plt.subplots(
        num_dimensions, 1, figsize=(15, 3 * num_dimensions), sharex=True
    )
...
def visualize_counterfactuals(ce, instance: int, optimized=False, precision=True):
    ...
    for i in range(num_dimensions):
        ax = (
            axes[i] if num_dimensions > 1 else axes
        )  # Handle case with a single dimension plot

        # Plot the original series
        (original_line,) = ax.plot(
            sample_reshaped[i], label="Original", color="#64cdc0", linewidth=2
        )
        ...
        if diffs.size > 0:  # If there are differences
            ...
            if start_diff > 0:  # Make sure there's a point to connect from
                ax.plot(
                    [start_diff - 1, start_diff],
                    [
                        sample_reshaped[i, start_diff - 1],
                        counterfactual_reshaped[i, start_diff],
                    ],
                    color="salmon",
                    linewidth=2,
                )
            ...
            if (
                end_diff < sample_reshaped[i].size - 1
            ):  # Make sure there's a point to connect to
                ax.plot(
                    [end_diff, end_diff + 1],
                    [
                        counterfactual_reshaped[i, end_diff],
                        sample_reshaped[i, end_diff + 1],
                    ],
                    color="salmon",
                    linewidth=2,
                )
...
def plot_with_cam_as_line(
    ce, instance: int, optimized=False, precision=True, weights=None
):
    """
    ...
    sample = ce.X_test[instance]
    counterfactual = (
        ce.get_optimized_counterfactual(instance, precision)
        if optimized
        else ce.get_naive_counterfactual(instance)
    )
    ...
    data_dict = {
        f"Original_Dim_{i + 1}": sample_reshaped[i] for i in range(num_dimensions)
    }
    data_dict.update(
        {
            f"Counterfactual_Dim_{i + 1}": counterfactual_reshaped[i]
            for i in range(num_dimensions)
        }
    )
    if weights is not None:
...
def plot_with_cam_as_heatmap(
    ...
    if weights is not None:
        weights = np.interp(
            weights, (weights.min(), weights.max()), (0, 1)
        )  # Normalize for heatmap

    plt.style.use("seaborn-v0_8-darkgrid")
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(15, 4.5), sharex=True, gridspec_kw={"height_ratios": [3, 0.5]}
    )
    ...
    diffs = np.where(sample_reshaped[dimension] != counterfactual_reshaped[dimension])[
        0
    ]
    if diffs.size > 0:
        start_diff, end_diff = diffs[0], diffs[-1]
        ...
        if start_diff > 0:
            ax1.plot(
                [start_diff - 1, start_diff],
                [
                    sample_reshaped[dimension, start_diff - 1],
                    counterfactual_reshaped[dimension, start_diff],
                ],
                color=cf_color,
                linewidth=2,
            )
        if end_diff < sample_reshaped[dimension].size - 1:
            ax1.plot(
                [end_diff, end_diff + 1],
                [
                    counterfactual_reshaped[dimension, end_diff],
                    sample_reshaped[dimension, end_diff + 1],
                ],
                color=cf_color,
                linewidth=2,
            )
...
def plot_method_comparison_with_cam_horizontal(
    sample: np.ndarray, counterfactuals: dict, weights: np.ndarray, dimension: int = 0
) -> None:
    """
    ...
    cf_colors = {"CONFETTI": "#ff595a", "CoMTE": "#2660A4", "TSEvo": "#F3B700"}
    ...
    for i, method in enumerate(methods):
        ...
        if diffs.size > 0:
            start_diff, end_diff = diffs[0], diffs[-1]
            ax_ts.plot(
                timesteps[start_diff : end_diff + 1],
            ...
            if start_diff > 0:
                ax_ts.plot(
                    [timesteps[start_diff - 1], timesteps[start_diff]],
            ...
            if end_diff < len(sample_dim) - 1:
                ax_ts.plot(
                    [timesteps[end_diff], timesteps[end_diff + 1]],
...
def tradeoff_sparsity_proximity(df):
    ...
    plt.scatter(
        df["Proximity"], df["Sparsity"], color="#64cdc0", alpha=0.7, label="Samples"
    )
