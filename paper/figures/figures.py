...
# Load results
confetti_fcn_bm = pd.read_csv(
    counterfactuals_path / f"confetti_optimized_{dataset}_{model_name}_theta_0.85.csv"
)
comte_fcn_bm = pd.read_csv(
    counterfactuals_path / f"comte_{dataset}_{model_name}_counterfactuals.csv"
)
sets_fcn_bm = pd.read_csv(
    counterfactuals_path / f"sets_{dataset}_{model_name}_counterfactuals.csv"
)
tsevo_fcn_bm = pd.read_csv(
    counterfactuals_path / f"tsevo_{dataset}_{model_name}_counterfactuals.csv"
)

# Transform results into arrays
confetti_fcn_bm["Solution"] = confetti_fcn_bm["Solution"].apply(
    lambda x: convert_string_to_array(x, timesteps=100, channels=6)
)
comte_fcn_bm["Solution"] = comte_fcn_bm["Solution"].apply(
    lambda x: convert_string_to_array(x, timesteps=100, channels=6)
)
sets_fcn_bm["Solution"] = sets_fcn_bm["Solution"].apply(
    lambda x: convert_string_to_array(x, timesteps=100, channels=6)
)
tsevo_fcn_bm["Solution"] = tsevo_fcn_bm["Solution"].apply(
    lambda x: convert_string_to_array(x, timesteps=100, channels=6)
)
