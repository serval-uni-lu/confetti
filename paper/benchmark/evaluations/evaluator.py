...
class Evaluator:
    ...
    This class assumes that the result's dataframe or csv file contains at least the following columns:
        - 'Test Instance': Index of the original instance in the sample set.
        - 'Solution': The counterfactual solution as a numpy array or string representation.
        - 'CE Label': The predicted label of the counterfactual instance.
    ...
    def evaluate_from_csv(
        ...
        optional_path: Optional[Union[Path,str]] = None,
        ...
        model_name (str):
            The name of the model that was used. Either 'fcn' or 'resnet'.
        alpha (bool):
            Only applies when explainer = 'confetti'. If True, loads alpha solutions, otherwise loads theta solutions.
        param_config (float):
        Only applies when explainer = 'confetti'. The parameter configuration used for the explainer.
        ...
        X_sample, y_sample = load_multivariate_ts_from_csv(
            f"{cfg.DATA_DIR}/{dataset}_{model_name}_samples.csv"
        )  # Instances that were explained
        ...
        # Get Counterfactuals
        counterfactuals = self.__get_counterfactuals(
            explainer, dataset, model_name, alpha, param_config, optional_path
        )

        counterfactuals["Solution"] = counterfactuals["Solution"].apply(
        ...
        # Compute Metrics
        counterfactuals_metrics = self.__compute_metrics(
            model=model,
        ...
        dataset_summary = self.__compute_dataset_summary(
            explainer=explainer,
            dataset=dataset,
            sample=X_sample,
            counterfactuals=counterfactuals,
    ...
    def evaluate_results(
        ...
        Args:
            ...
            param_config (float): Only applies when explainer = 'confetti'. The parameter configuration used for the hyperparameter
    ...
    def __compute_metrics(
        ...
        for i in range(len(sample)):
            ...
            if i in counterfactuals["Test Instance"].values:
                ...
                instance_counterfactuals = counterfactuals[
                    counterfactuals["Test Instance"] == i
                ]
                for _, counterfactual in instance_counterfactuals.iterrows():
                    ...
                    # Compute Confidence
                    confidence = self.__get_confidence(
                        model, ce, timesteps, channels, original_label
                    )
                    ...
                    plausibility = self._yNN_timeseries(
                        counterfactual=ce,
                        model=model,
                        data=training_data,
                        k=5,
                ...
                print(
                    f"No counterfactual found for instance {i} in dataset. Skipping..."
                )
                continue

        return metrics

    def __compute_dataset_summary(
        self,
        explainer: str,
        dataset: str,
        sample: np.ndarray,
        ...
        metrics (pd.DataFrame): The computed metrics for each counterfactual.
        alpha (bool): Only applies when explainer = 'confetti'. If True, loads alpha solutions, otherwise loads
        param_config (float): Only applies when explainer = 'confetti'. The parameter configuration used for
            the explainer.
    ...
    @staticmethod
    def __get_model(dataset: str, model: str):
        """
    ...
    def __get_counterfactuals(
        ...
        Loads counterfactuals. Supported 'explainer' values:
          comte, sets, tsevo, confetti, ablation_study
        """
        ...
        if explainer in {"confetti", "ablation_study"}:
            ...
            v = float(param_config)
            candid_strs = [str(param_config), f"{param_config:g}"]
            if v.is_integer():
                candid_strs.append(f"{int(v)}.0")  # ensures 0.0, 1.0
                candid_strs.append(str(int(v)))  # fallback 0, 1
    ...
    @staticmethod
    def __get_coverage(
        sample: np.ndarray, counterfactuals: pd.DataFrame
    ) -> float:
        """
    ...
    @staticmethod
    def __get_sparsity(original: np.array, counterfactual: np.array):
        """
    ...
    def __get_confidence(
        model, counterfactual, timesteps, channels, original_label
    ):
        """
    ...
    def __get_validity(
        ...
        ce_label = np.argmax(
            model.predict(counterfactual.reshape(1, timesteps, channels))
        )
        return 0 if original_label == ce_label else 1

    @staticmethod
    def __l1_distance(ts1: np.ndarray, ts2: np.ndarray):
        """
        ...
        if ts1.shape != ts2.shape:
            raise ValueError(
                "Time series must have the same shape (Timesteps, Dimensions)."
            )
        return np.sum(np.abs(ts1 - ts2))

    @staticmethod
    def __l2_distance(ts1: np.ndarray, ts2: np.ndarray):
        """
    ...
    @staticmethod
    def __dynamic_time_warping(ts1: np.ndarray, ts2: np.ndarray) -> float:
        """
    ...
    def _yNN_timeseries(
        ...
        if label is None:
            label = np.argmax(
                model.predict(
                    counterfactual.reshape(1, *counterfactual.shape), verbose=0
                ),
                axis=1,
            )
        ...
        nn_idx = nbrs.kneighbors(
            counterfactual.reshape(1, T, D), return_distance=False
        )[0]
